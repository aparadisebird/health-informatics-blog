...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        // This specifically looks for posts with the category 'Project'
        const q = query(
          collection(db, "posts"), 
  ...
  return (
    <main className="min-h-screen bg-[#fdfcfb]">
      ...
      {/* Header */}
      <header className="max-w-6xl mx-auto py-20 px-6">
        <div className="inline-flex items-center gap-3 mb-6 px-4 py-1 bg-amber-50 border border-amber-100 rounded-full">
        ...
        <p className="text-xl text-stone-600 font-medium max-w-2xl leading-relaxed">
          A collection of digital tools, research frameworks, and technical implementations developed to solve public health challenges.
        </p>
      </header>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-32">
        {loading ? (
          ...
          <div className="py-20 text-center bg-white border border-dashed border-stone-200 rounded-[3rem]">
             <p className="text-stone-400 italic">No projects published yet. Use the Admin panel to add a 'Project' category post.</p>
          </div>
        ) : (
            ...
            {projects.map((p) => (
              <Link key={p.id} href={`/posts/${p.slug}`} className="group bg-white border border-stone-100 rounded-[3rem] p-10 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col">
                ...
                <p className="text-stone-500 leading-relaxed mb-8 flex-1">
                  Technical documentation regarding the architecture, data processing, and clinical impact of this informatics project.
                </p>
